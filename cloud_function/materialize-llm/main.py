...
CSV_COLUMNS = [
    ...
    "price", "year", "make", "model", "mileage","transmission", "fuel_type", "condition", "title_status",
    "location", "seller_type", "color", "zip_code",
    "source_txt"
]
...
def _jsonl_records_for_run(bucket: str, structured_prefix: str, run_id: str):
    ...
    prefix = f"{structured_prefix}/run_id={run_id}/jsonl_llm/" # changes jsonl to jsonl_llm
...
def materialize_http(request: Request):
    ...
    try:
        ...
        run_ids = _list_run_ids(BUCKET_NAME, STRUCTURED_PREFIX)[-200:]# start with the most recent 200 runs
        if not run_ids:
        ...
        final_key = f"{base}/listings_master_llm.csv" # Added llm
